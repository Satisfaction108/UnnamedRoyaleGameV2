...
export default class Game {
  constructor(players) {
    ...
    this.speed = 300 // world units/sec
    this.state = new Map()
    this.inputs = new Map()
    this.closed = false
    ...
    console.log(
      `[GAME ${this.id}] created with players: ${players
        .map(p => p.name || p.id)
        .join(' vs ')}`
    )
    ...
    players.forEach((p, i) => {
      ...
      // tell each client the match started
      p.ws.send(
        JSON.stringify({
          type: 'matchStart',
          gameId: this.id,
  ...
  tick() {
    ...
    this.players.forEach(p => {
      try {
        p.ws.send(JSON.stringify({ type: 'matchEnd', reason }))
      } catch {}
      try {
